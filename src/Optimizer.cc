...
/*
 * Pose Optimization function without adjust the Map Points
 */
 ...
 * Pose Optimization function without adjust the Map Points
 */
    int Optimizer::PoseOptimization_dynamic(Frame *pFrame) {
        //geo preparation
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
        linearSolver = new g2o::LinearSolverDense<g2o::BlockSolver_6_3::PoseMatrixType>();
...

        {
            ...
            for (int i = 0; i < N; i++) {
                MapPoint *pMP = pFrame->mvpMapPoints[i];
                if (pMP) {
                    if (!pFrame->mvKeysDynamic[i]) {///adds on
                        // Monocular observation
                        if (pFrame->mvuRight[i] < 0) {
                            nInitialCorrespondences++;
                            pFrame->mvbOutlier[i] = false;
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap) {
        ...
        pKF->mnBALocalForKF = pKF->mnId;//mark, for which KF this KF is used for local BA
        ...
        //Store local mappoint from local keyframes
        ...
        // Fixed Keyframes. Some Other Keyframes that see Local MapPoints but that are not Local Keyframes
        ...
        for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
            ...
            for (map<KeyFrame *, size_t>::iterator mit = observations.begin(), mend = observations.end();
                ...
                //the observing KF is not used for this KF's BA. and not fixed for this KF's BA
                if (pKFi->mnBALocalForKF != pKF->mnId && pKFi->mnBAFixedForKF != pKF->mnId) {
                    pKFi->mnBAFixedForKF = pKF->mnId;//fix this
                    if (!pKFi->isBad())
        ...
        for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
            ...
            for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                if (!pKFi->isBad()) {
                    if(!pKFi->mvKeysDynamics[mit->second]){///adds on
