...
parser = argparse.ArgumentParser(description="Evaluate trajectory using evo_ape")
...
base_gt = "/xinfolder/Bonn"
base_result = "/home/xin/Downloads/ORB_SLAM2/Examples/RGB-D/results/revised-result/Bonn2"
...
# 保存为 txt
out_file = Path(f"{dataset_name}_rpe_summary_trans_part.txt")
...
for i in range(1, 21):
    traj_file = f"{base_result}/{dataset_name}/CameraTrajectory_{i}.txt"
    cmd = [
        "evo_rpe", "tum", gt_file, "-vas", traj_file, "-r", "angle_deg"
    ]
    print("Running:", " ".join(cmd))
...
# 保存为 txt
out_file = Path(f"{dataset_name}_rpe_summary_rot_part.txt")
np.savetxt(out_file, matrix, fmt="%.6f")
