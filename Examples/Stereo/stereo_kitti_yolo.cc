...
int main(int argc, char** argv)
{
    if (argc != 5)
    {
        cerr << endl << "Usage: ./stereo_kitti path_to_vocabulary path_to_settings path_to_sequence metric_type" <<
            endl;
        cerr << "metric_type: variance | euclidean" << endl;
        cerr <<
            "Example: ./stereo_kitti ../Vocabulary/ORBvoc.txt ../Examples/RGB-D/Kitti.yaml ../kitti_sequence variance"
            << endl;
        return 1;
    }
    ...
    ///metric method
    LoadClasses(vstrImageLeft, vstrClassFilenames, string(argv[3]));
    string metric_type = string(argv[4]);
    ...
    //todo check if load image correctly and calc optical flow correctly!
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        SLAM.TrackStereo(imLeft, imRight, tframe, vstrClassFilenames[ni]);
...
{
    vstrClassFilenames.resize(vstrImageFilenames.size());
    for (int i = 0; i < vstrImageFilenames.size(); i++)
    {
        const string &path = vstrImageFilenames[i];
        // 找到最后的 '/' 和 '.' 的位置
        ...
        if (slash == string::npos)
            filename = (dot == string::npos) ? path : path.substr(0, dot);
        else
            filename = (dot == string::npos) ? path.substr(slash + 1)
                                             : path.substr(slash + 1, dot - slash - 1);

        // 注意这里加了 "/mask/"
        vstrClassFilenames[i] = folder + "/mask/" + filename + "_class.txt";
    }
}
