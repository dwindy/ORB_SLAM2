...
void LoadImages(const string &strSequence, vector<string> &vstrImageFilenames, vector<string> &vstrImageFilenamesRight,vector<double> &vTimestamps);
...
int main(int argc, char **argv)
{
    ...
    cout << "image path " << argv[3] << endl;
    //LoadImages(string(argv[3]), vstrImageFilenames, vTimestamps);
    LoadImages(string(argv[3]), vstrImageFilenames, vstrImageFilenames_right, vTimestamps);
    int nImages = vstrImageFilenames.size();
    cout << "load image done " << endl;
    ///Added Module
    //Retrieve paths to Laser Scans
    vector<string> vstrScanFilenames;
    cout << "lidar path  " << argv[4] << endl;
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        ///Added Module
        im_right = cv::imread(vstrImageFilenames_right[ni], CV_LOAD_IMAGE_UNCHANGED);
...
//        vector<double> laserTimes = {vLaserTimestamps[ni], vLaserStartTimes[ni], vLaserEndTimes[ni]};
        ...
        SLAM.TrackMonoLiDAR(im, im_right, tframe, laserPoints);
 ...
 */
void
LoadLaserscansRaw(const string &strPathToSequence, vector<string> &vstrLaserscanFilenames, vector<double> &vTimestamps,
                  vector<double> &vTimestarts, vector<double> &vTimeends) {
    //load scan times
    ifstream fTimes;
