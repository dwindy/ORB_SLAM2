...
///Added Module
    class LineAndPlaneMatcher {
    public:
        LineAndPlaneMatcher(){}
        LineAndPlaneMatcher(double angleIn, double midIn, double lengthIn):lineAngleThreshold(angleIn),lineMidDisThreshold(midIn),lineLengthThreshold(lengthIn){}
        double lineAngleThreshold;
        double lineMidDisThreshold;
        double lineLengthThreshold;

        int LineMatching(vector<mLine> lines0, vector<mLine> lines1,
                          cv::Mat descriptors0, cv::Mat descriptors1,
                          vector<vector<int>> &resultMatches);
        int LineMatchingByKeyframe(Frame *CurrentFrame, KeyFrame *pKF, vector<vector<int>> &resultMatches);
        int LineMatchingByProjection(Frame *CurrentFrame, Frame *LastFrame, vector<vector<int>> &resultMatches);
        // Search matches between Frame keypoints and projected MapPoints. Returns number of matches
        // Used to track the local map (Tracking)
        int SearchLineByProjection(Frame &F, const std::vector<MapLine *> &vpMapLines, const float th = 3);
    };
